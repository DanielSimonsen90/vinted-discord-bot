...
class Logger {
  ...
  static debug(message) {
    if (devMode) Logger.log('DEBUG', message, chalk.magenta);
  }
  ...
  static log(level, message, colorFn, includeSource = false) {
    ...
    if (dumpLogs) fs.appendFileSync(Logger.logFilePath, `${logMessage}\n`);
  }
  ...
  static getCallSource() {
    ...
    // Get the relevant caller line from stack; Adjust this number based on where this method is within your stack trace
    return stackLines[4]?.replace(/^\s+at\s+/g, '') ?? 'Unknown caller';
  }
}
